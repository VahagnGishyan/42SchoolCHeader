...
void Test_CSArrayInt::ResizeClear()
{
    ...
    if (strs1.resize(&strs1, -2) != false || strs1.resize(&strs1, 2) != false)
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
    ...
    /*Check 2*/
    int arrs2[4]{ 4 };
    strs2.set_data(&strs2, arrs2, 1, 4);
    ...
    if (strs3.resize(&strs3, -2) != 0 || strs3.resize(&strs3, 14) != 0)
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
	...
	std::cout << "\tstrs.resize(14) = " << strs3.resize(&strs3, 14) << "\t:: must be 0" << std::endl;
    }
    ...
    /*Check 5*/
    int arrd0[1];
    strd0.set_data(&strd0, arrd0, 0, 1);
    strd0.clear(&strd0);
    ...
    /*Check 6*/
    int arrd1[1]{4};
    strd1.set_data(&strd1, arrd1, 1, 1);
    strd1.clear(&strd1);
