...
void Test_CSArrayInt::ArraySize()
{
    std::cout << "\tArraySize()\t\t";
    ...
    c_sarray_int_constructor(&strs3);
    ...
    /*Check 0*/
    int arrs0[1];
    strs0.set_data(&strs0, arrs0, 0, 1);
    if (strs0.size(&strs0) != 0 || strs0.capacity(&strs0) != 1 || !strs0.is_empty(&strs0))
    {
	result = false;
	std::cout << "ERROR" << std::endl;
	std::cout << "\tCheck 0\t" << std::endl;
	std::cout << "\tstrs.size()      = " << strs0.size(&strs0) << "\t:: must be 0" << std::endl;
	std::cout << "\tstrs.capasity()  = " << strs0.capacity(&strs0) << "\t:: must be 1" << std::endl;
	std::cout << "\t!strs.is_empty() = " << !strs0.is_empty(&strs0) << "\t:: must be false" << std::endl;
    }

    /*Check 1*/
    int arrs1[1]{ 4 };
    strs1.set_data(&strs1, arrs1, 0, 1);
    strs1.clear(&strs1);
    if (strs1.size(&strs1) != 0 || strs1.capacity(&strs1) != 1 || !strs1.is_empty(&strs1))
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
	...
	std::cout << "\tstrs.capasity()  = " << strs1.capacity(&strs1) << "\t:: must be 1" << std::endl;
	std::cout << "\t!strs.is_empty() = " << !strs1.is_empty(&strs1) << "\t:: must be false" << std::endl;
    }

    /*Check 2*/
    int arrs2[1]{ 4 };
    strs2.set_data(&strs2, arrs2, 1, 128);
    strs2.clear(&strs2);
    if (strs2.size(&strs2) != 0 || strs2.capacity(&strs2) != 128 || !strs2.is_empty(&strs2))
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
	...
	std::cout << "\tstrs.capasity()  = " << strs2.capacity(&strs2) << "\t:: must be 128" << std::endl;
	std::cout << "\t!strs.is_empty() = " << !strs2.is_empty(&strs2) << "\t:: must be false" << std::endl;
    }

    /*Check 3*/
    int arrs3[128]{ 0, 1, 2, 3, 4 };
    strs3.set_data(&strs3, arrs3, 5, 128);
    strs3.clear(&strs3);
    if (strs3.size(&strs3) != 0 || strs3.capacity(&strs3) != 128 || !strs3.is_empty(&strs3))
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
