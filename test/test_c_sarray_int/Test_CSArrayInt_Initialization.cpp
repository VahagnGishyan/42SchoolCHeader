...
void Test_CSArrayInt::Initialization()
{
    ...
    /*Check 2*/
    int arrs2[1]{ 4 };
    strs2.set_data(&strs2, arrs2, 1, 1);
    char symbol_front2 = *strs2.front(&strs2);
    char symbol_back2 = *strs2.back(&strs2);
    if (strs2.size(&strs2) != 1 || strs2.capacity(&strs2) != 1 || symbol_front2 != 4 || symbol_back2 != 4)
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
    ...
    if (strs3.size(&strs3) != 5 || strs3.capacity(&strs3) != 128 || *strs3.front(&strs3) != 0 || *strs3.back(&strs3) != 4)
    {
	if (result)
	{
	    std::cout << "ERROR" << std::endl;
