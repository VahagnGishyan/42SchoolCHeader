...
void Test_CSArrayString::IndexOperators()
{
    ...
    strs0.set_data(&strs0, arrsorc, 4, 8);
    /*Check 0*/
    if (c_string_compare_chararray(strs0.at(&strs0, 0), "", 0))
    {
	if (result)
	{
	...
	std::cout << "\tc_string_compare_chararray(strs0.at(&strs0, 0), "", 0)     = " 
	    << c_string_compare_chararray(strs0.at(&strs0, 0), "", 0) << "\t:: must be 0" << std::endl;
    }
