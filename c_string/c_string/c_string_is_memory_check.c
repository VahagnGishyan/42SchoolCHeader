...
{
    return (!(str->is_dynamic_memory_null(&str->m_dynamic_array))  /* is dmemory char is not null */
	&& str->m_dynamic_capacity				    /* is dmemory capacity is not 0 */
	&& (str->m_size > str->m_static_capacity));		    /* is size > static capacity */
}
