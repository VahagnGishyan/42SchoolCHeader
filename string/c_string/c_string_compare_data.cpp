...
int c_string_compare_chararray(t_c_string* str, char* arr, int size)
{
    ...
    if (!c_string_is_used_dynamic_memory(str) || (str->m_size + size) < str->m_static_array.m_capacity)
    {
	return(str->m_static_array.compare_chararray(&str->m_static_array, arr, size));
    }
